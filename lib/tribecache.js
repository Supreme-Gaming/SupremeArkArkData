  ...
  return new Promise((r, rj) => {
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        connection.query('CREATE TABLE IF NOT EXISTS tribes ( `Id` INTEGER  NOT NULL, `Guid` VARCHAR(255) NOT NULL UNIQUE, `Name` VARCHAR(255) NOT NULL, `OwnerId` INT NULL, `FileCreated` BIGINT NULL, `FileUpdated` BIGINT NULL, `TribeLog` MEDIUMTEXT NULL, `PlayMap` VARCHAR(255) NULL, `Host` VARCHAR(255) NULL, `DataPort` INT NULL)', (err, res) => {
          if (err) {
            connection.release();
            rj(err);
          }
