...
function initTable() {
    return new Promise(function(resolve, reject) {
        ...
        db.serialize(function() {
            ...
            db.run('CREATE TABLE if not exists players (Id INTEGER  NOT NULL UNIQUE, TribeId INT NULL, TribeName VARCHAR NULL, Level INT NOT NULL, Engrams INT NOT NULL, SteamId VARCHAR NOT NULL UNIQUE, Admin bool NOT NULL  DEFAULT false, CharacterName VARCHAR NULL,	SteamName VARCHAR NULL,ProfileUrl VARCHAR NULL,	AvatarUrl VARCHAR NULL,	CommunityBanned INT NULL,	VACBanned INT NULL,	NumberOfVACBans INT NULL,	NumberOfGameBans INT NULL,	DaysSinceLastBan INT NULL,  Banned bool NOT NULL  DEFAULT false,FileUpdated DATETIME NULL, FileCreated DATETIME NULL, PlayMap VARCHAR NULL, Host VARCHAR NULL, DataPort INT NULL)', function(e) {
                if (e) {
                ...
                checkId(item.Id, function(d) {
                    if (d === undefined && item.Id !== false) {
                        db.parallelize(function() {
                            db.run("INSERT INTO players (id,steamid,charactername,level,engrams,tribeid,tribename,banned,admin,fileupdated,filecreated,playmap,host,dataport) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [item.Id, item.SteamId, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.TribeName, item.Banned, item.Admin, item.FileUpdated, item.FileCreated, item.PlayMap, item.Host, item.DataPort], function(err, sql) {
                                if (err) {
                                    db.run("Delete from players where steamid = ?", item.SteamId);
                                    console.log("LINE 55:", err, "\n Will attempt to fix broken cache record...");
                                    db.run("INSERT INTO players (id,steamid,charactername,level,engrams,tribeid,tribename,fileupdated,banned,admin,filecreated,playmap,host,dataport) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [item.Id, item.SteamId, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.TribeName, item.Banned, item.Admin, item.FileUpdated, item.FileCreated, item.PlayMap, item.Host, item.DataPort], function(err, sql) {
