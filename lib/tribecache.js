...
function initTable() {
    return new Promise((r, rj) => {
        db.run("drop table if exists tribes");
        db.run('CREATE TABLE IF NOT EXISTS tribes ( Id INTEGER  NOT NULL UNIQUE,Name VARCHAR NOT NULL,OwnerId INT NULL,	FileCreated DATETIME NULL,FileUpdated DATETIME NULL,TribeLog VARCHAR NULL, PlayMap VARCHAR NULL, Host VARCHAR NULL, DataPort INT NULL)', (err, sql) => {
            r();
        });
    });
                    ...
                    .then((d) => {
                        if (d === undefined && item.Id !== false) {
                            db.parallelize(function() {
                                db.run("INSERT INTO tribes (Id,Name,OwnerId,FileCreated,FileUpdated,TribeLog,PlayMap,Host,DataPort) VALUES (?,?,?,?,?,?,?,?,?)", [item.Id, item.Name, item.OwnerId, item.FileCreated, item.FileUpdated, item.TribeLog, item.PlayMap, item.Host, item.DataPort],
                                    function(err, sql) {
                                        if (err) {
...
module.exports.setupTribes = function() {
    return new Promise((r, rj) => {
        initTable()
            .then(() => readDirPromisified(path.join(path.normalize(server_settings.ark_path), "ShooterGame", "Saved", server_settings.map_folder_name), "utf-8"))
            .then((files) => {
                var players = [];
                var tribeData = {};
                qrylist = [];
                let reqs = files.map((v) => {
                    return new Promise(function(resolve) {var re = new RegExp("^.*\\.arktribe");
                        ...
                        if (re.test(v)) {
                            ...
                            var fdata = fs.statSync(path.join(server_settings.ark_path, "ShooterGame", "Saved", server_settings.map_folder_name, v));
                            ...
                            tribeData.PlayMap = server_settings.map_name;
                            tribeData.Host = settings.sourcequery.host;
                            tribeData.DataPort = server_settings.port;
