...
initTable = () => {
  return new Promise(function(r, rj) {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query('create table if not exists players (`Id` INTEGER NOT NULL UNIQUE, `TribeId` INT NULL, `TribeName` VARCHAR(255) NULL, `Level` INT NOT NULL, `Engrams` INT NOT NULL, `SteamId` VARCHAR(255) NOT NULL UNIQUE, `Admin` INT NOT NULL DEFAULT false, `CharacterName` VARCHAR(255) NULL,	`SteamName` VARCHAR(255) NULL, `ProfileUrl` VARCHAR(255) NULL, `AvatarUrl` VARCHAR(255) NULL, `CommunityBanned` INT NULL, `VACBanned` INT NULL, `NumberOfVACBans` INT NULL, `NumberOfGameBans` INT NULL, `DaysSinceLastBan` INT NULL, `Banned` INT NOT NULL DEFAULT false, `FileUpdated` DATETIME NULL, `FileCreated` DATETIME NULL, `PlayMap` VARCHAR(255) NULL, `Host` VARCHAR(255) NULL, `DataPort` INT NULL) DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci', function(err, res, fields) {
        connection.release();
...
savePlayers = (data) => {
  ...
  return new Promise(function(resolve, reject) {
    ...
    pool.getConnection((err, connection) => {
      ...
      reqs = data.map((item) => {
        return [item.Id, item.SteamId, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.TribeName, item.Banned, item.Admin, item.FileUpdated, item.FileCreated, item.PlayMap, item.Host, item.DataPort]
      });

      connection.query("INSERT INTO `supremeark_arkdata`.`players` (id,steamid,charactername,level,engrams,tribeid,tribename,banned,admin,fileupdated,filecreated,playmap,host,dataport) VALUES ?", [reqs], function(err, res) {
        if (err) {
