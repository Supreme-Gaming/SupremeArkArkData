  ...
  return new Promise((r, rj) => {
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        connection.query('CREATE TABLE IF NOT EXISTS tribes ( `Id` INTEGER  NOT NULL, `Guid` VARCHAR(255) NOT NULL UNIQUE, `Name` VARCHAR(255) NOT NULL, `OwnerId` INT NULL, `FileCreated` BIGINT NULL, `FileUpdated` BIGINT NULL, `TribeLog` MEDIUMTEXT NULL, `PlayMap` VARCHAR(255) NULL, `Host` VARCHAR(255) NULL, `DataPort` VARCHAR(255) NULL)', (error, res) => {
          if (error) {
              ...
              let reqs = files.map((v) => {
                return new Promise(function(resolve) {
                  ...
                  if (re.test(v)) {
                    ...
                    tribeData.Guid = parser.getInt("TribeID", data) + '-' + server.port;
