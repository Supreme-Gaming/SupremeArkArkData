        ...
        readFilePromisified(path.join(path.normalize(server.server_directory), "ShooterGame", "Binaries", "Linux", "BanList.txt"), "ucs2")
          ...
          .then((files) => {
            ...
            let reqs = files.map((v) => {
              ...
              if (re.test(v)) {
                var data = fs.readFileSync(path.join(path.normalize(server.server_directory), "ShooterGame", "Saved", server.server_alt_dir, v));
                playerData = {};
                playerData.Id = getId(data);
                playerData.SteamId = getSteamId(data);
                playerData.Guid = getSteamId(data) + '-' + server.rconport;
                ...
                if (parser.getInt("TribeID", data) == false) {
                  ...
                  playerData.TribeGuid = parser.getInt("TribeID", data) + '-' + server.rconport;
                }
                ...
                if (playerData.TribeId > 0) {
                  if (fs.existsSync(path.join(path.normalize(server.server_directory), "ShooterGame", "Saved", server.server_alt_dir, playerData.TribeId + '.arktribe'))) {
                    var tribeData = fs.readFileSync(path.join(path.normalize(server.server_directory), "ShooterGame", "Saved", server.server_alt_dir, playerData.TribeId + '.arktribe'));
                    playerData.TribeName = parser.getString("TribeName", tribeData);
                  } else {
                    playerData.TribeName = 'null';
                  }
                }
                var fdata = fs.statSync(path.join(path.normalize(server.server_directory), "ShooterGame", "Saved", server.server_alt_dir, v));
                ...
                playerData.Host = server.rconport;
