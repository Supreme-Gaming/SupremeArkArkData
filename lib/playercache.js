...
savePlayers = (data) => {
  ...
  return new Promise(function(resolve, reject) {
    ...
    const template = "INSERT INTO `supremeark_arkdata`.`players` (id,steamid,guid,charactername,level,engrams,tribeid,tribename,banned,admin,fileupdated,filecreated,playmap,host,dataport) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE charactername=?,level=?,engrams=?,tribeid=?,tribename=?,banned=?,admin=?,fileupdated=?,playmap=?,host=?,dataport=?; ";

    let query = ""

    data.forEach(item => {
      query += mysql.format(template, [item.Id, item.SteamId, item.Guid, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.TribeName, item.Banned, item.Admin, item.FileUpdated, item.FileCreated, item.PlayMap, item.Host, item.DataPort, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.TribeName, item.Banned, item.Admin, item.FileUpdated, item.PlayMap, item.Host, item.DataPort]);
    })
